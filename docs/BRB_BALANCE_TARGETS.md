...
- **14-strategy matrix (with `advisor_dependent`):** 2 coups (0.04%), 0 cabals. The dependent profile — consults its highest-leverage advisor every month, never disciplines advisors, restocks Intelligence to protect the habit — reaches Leverage 90 regularly, but the unchanged departure rule usually fires first because Institutions are still above 40 at that moment; the coup lands only when institutional decay coincides with the leverage cap. In isolated 200-run probes the profile produces ~1.5% coups and 0 activations (its runs end ~92% in Corporation capture or collapse). The cabal has no natural constituency yet: a single concentrating profile never lifts two advisors to 70 simultaneously.

The endings are engine-verified by unit tests and now measurably alive in the baseline. Do not loosen the thresholds just to force higher simulation counts; the coup firing rarely — and only against an already-degraded state — is the intended shape. Human playtests remain the real measure of how often over-reliance happens organically.
