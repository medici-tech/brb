...
Over-reliance now has two dedicated loss endings driven by `ADVISOR_TAKEOVER_RULES`. The coup Leverage bar is **decoupled from and below the departure bar**: an advisor departs on their own terms at Leverage 90 only when the state is *not* dependent, and seizes control at Leverage **80** when it is. An **Advisor Coup** fires when one advisor is at Leverage 80 while Institutions are at 65 or below, or while no other advisor remains active. An **Advisor Cabal** fires when two or more active advisors each hold Leverage 50 or above. Both outrank the all-advisors-gone State Collapse but yield to Corporation victory and Panic/Institutions collapse in the same month.
...
- **Current checkpoint — 15-strategy matrix, coup bar decoupled to 80 / Institutions 65, cabal 50, `advisor_cabal` profile added:** **150 coups (3.0%), 334 cabals (6.68%)** — a combined ~9.7% of runs. Two changes unlocked this. (1) Lowering the coup Leverage bar to 80, below the 90 departure, meant the dependent profile's plateau (leverage always reaches 80, but Institutions sit ~63) now converts to coups once the Institutions bar rose to 65. (2) The concentrating dependent bot always triggers a *coup* before a cabal, so cabals needed a dedicated spreader: `advisor_cabal` consults the lower of its two most-leveraged advisors, lifting a pair in step past 50 without either reaching coup territory. Aggregate wins dipped slightly (8.48% activation) and Corporate Capture fell to 44.78% because two advisor-heavy profiles now end in takeovers instead of losing to the Corporation.

`advisor_dependent` and `advisor_cabal` are deliberate demonstrator profiles (like `civic_seeker` for Civic Legacy): the dependent bot coups ~40% of its runs, the cabal bot cabals nearly all of its. They prove the endings are frequently reachable and calibrate the thresholds; the true organic rate for human play is still unknown and remains the real measure. Do not read the 3% / 6.68% aggregate as a human-play prediction — it is the rate for a matrix that intentionally includes two over-reliant strategies.
