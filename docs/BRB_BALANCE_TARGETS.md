...
Replay evidence is deliberately protected from the scar. A same-seed replay always opens unscarred, so a divergence never reports a Panic difference the player never chose. The scar is postponed rather than spent in that case, so it still lands on the next ordinary campaign. When reading free-play results, treat "did Panic +6 feel like meaningful pain?" as a question about the player's *ordinary* campaigns; the report now names the aftermath on any run that opened under it, which is the field to check before attributing a hard opening to it.
