...
- **Current checkpoint — thresholds tightened to 55/60:** 10 coups (0.2%), 1 cabal (0.02%). Raising the coup Institutions bar 40→55 and lowering the cabal member bar 70→60 made the takeovers roughly 5× more present while leaving the aggregate essentially unchanged (9.64% activation, 39.82% collapse, 50.32% capture — within noise of the 40/70 block). In isolated 200-run probes the dependent profile produces ~2.5% coups. The coup still fires only when institutional decay coincides with the leverage cap; the cabal remains rare because a concentrating dependent bot spends its leverage on one advisor, so cabals arise mainly when other strategies happen to drift two advisors past 60.

The endings are engine-verified by unit tests and measurably alive in the baseline. These thresholds make over-reliance bite harder without reshaping overall difficulty. Do not loosen them further just to force higher simulation counts; the coup landing only against an already-degraded state is the intended shape, and human playtests remain the real measure of how often over-reliance happens organically.
