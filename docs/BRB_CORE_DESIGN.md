...
Archive v1 unlocks limited power through six Legacy Directives. Clearance is progress to the next Directive unlock: a loss earns 1, Necessary Regime earns 2, and Civic Legacy earns 3. Every 3 Clearance produces a deterministic draft of up to three locked Directives. The chosen Directive remains permanently unlocked, but the player may equip only one and use it once in a campaign. Each benefit carries a visible political cost. A Necessary Regime also records a one-campaign aftermath scar: the next ordinary campaign opens with Panic +6, and the scar is spent on that campaign's first commitment rather than the moment it opens, so abandoning a run does not dodge it. Civic Legacy clears any pending scar. A same-seed replay is a controlled comparison, so it opens clean and holds the scar back for the next ordinary campaign. Together with Legacy Directives this is a bounded exception to the knowledge-only rule, not an open-ended stat ladder.
